...
| 功能 | 说明 |
|---|---|
| 🪟 **每轮即时推送** | 回答结束后，Windows 系统通知（toast）立即弹出本条消耗，两行紧凑布局：行1 = 模型名 + 时段标注 + 耗时，行2 = 输入 / 输出 + 缓存占比 + 费用 |
...
| ⏰ **时段价格标注** | 行1 显示 `高峰×2` 等时段策略（DeepSeek 原厂系峰谷定价）；其他模型统一定价不显示，避免噪音；预留夜间折扣字段 |
...
- **行1（标题大字）**：模型完整名 + 时段标注（`高峰×N`，仅峰谷定价模型高峰时显示）+ 耗时
- **行2（正文小字）**：输入 / 输出 + 缓存占比（两位小数）+ 费用（不带「约」字，未收录显示「未收录」）
- 宽度保护：极端场景（千万级数字 + 高价）实测 49u / 上限 52u，永远两行不换行
