...
### v2.22.0（2026-08-06）—— 内部调用按任务时间归属，多会话并发统计精确化

**修复的 Bug（重大）：多会话并发时 token 统计会串会话、丢消耗**

| 版本 | Bug | 根因 | 修复 |
|---|---|---|---|
| v2.19 | Stop toast 显示消耗远低于真实（曾把 744 tokens 当成本轮，真实 122.3 万） | Stop hook 触发比本轮 trace 落盘早 ~15ms，误把「会话起标题的内部小调用 trace」当本轮弹 toast | 入口 trace 非刚落盘（≤1s）时一律轮询等待「比入口更新」的有效 trace（≤3s）；超时且入口旧 → 标「上一轮」不冒充本条 |
| v2.20 | 一轮完整消耗只取最新一个 trace，内部调用丢失 | 一个用户轮次落盘多个 trace（起标题 744 + 主任务 122.3 万），旧实现只取最新 | 整轮聚合：`--hook` 记录本轮起点 `lastUserMsgAt`；`--stop` 聚合起点后同会话全部有效 trace |
| v2.21 | 多会话窗口同时打开时快照互相覆盖 → 聚合起点错乱 | `.snapshot.json` 全局单文件、不带 sessionId，后提交会话覆盖 `lastUserMsgAt` | 快照按 `session_id` 拆分（`.snapshot-<sid>.json`）；手动运行无 sid → 全局快照（行为不变） |
| v2.22 | 真并发交错时 A 会话的内部调用被 B 误收 | 无 sessionId 内部调用只看「起点之后」，B 在 A 任务中途提交时 A 的内部调用时间戳落在 B 起点后 | **最近主任务归属**：无 sid 内部调用归属到时间距离最近的主任务 trace（有 sessionId 的），仅归属本会话的才累加 |

**验证**：隔离测试 10/10 通过（含真并发交错：A 内部调用归 A、B 不误收；反向归属；单会话聚合回归）；真实混元 3.0 任务目录实测聚合与官方数据一分不差（in 1,190,538 / out 33,272 / cached 1,111,680 / 累计 1,223,810）。

**已知极限（诚实说明）**：两个会话的内部调用与各自主任务时间线**完全同秒重合**（毫秒级重叠）时仍无法区分——数据源未给内部调用打 sessionId 标记，属平台限制，现实中概率极低。
