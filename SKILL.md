...
脚本输出形如：
`⏱️ 耗时 3m 43s | 输入 287.9万 / 输出 1.2万 tokens（该轮累计 289.1万，缓存命中 287.2万）`

数字自动用「万/亿」单位（保留 1 位小数），低于 1 万显示原值，方便速读。
...
- **Windows 系统通知（toast，两行，Windows 通知默认只显示两行）**：调用 PowerShell WinRT Toast API 弹出
  ```
  ⏱️ 44.7s ｜ 输入 84.5万 ／ 输出 4106
  缓存 84.4万 ｜ 费用约 ¥0.05（高峰价）
  ```
  ——这是当前唯一确认有效的"本条可见"通道（UI 内 `systemMessage` 通道实测不显示，已放弃）。
